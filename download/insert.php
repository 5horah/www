...
<?
	...
	@extract($_GET);
	@extract($_SESSION);
	//수정=>mode=modify&num=7&page=1 / $subject=제목 / $content=내용글
	...
	// 다중 파일 업로드
	$files = $_FILES["upfile"];
	$count = count($files["name"]);	//count 첨부된 파일 개수		
	$upload_dir = './data/';
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		$file = explode(".", $upfile_name[$i]); //a1.jpg(explode 구분)
		$file_name = $file[0]; //a1
		$file_ext  = $file[1]; //jpg
		...
		if (!$upfile_error[$i]) //파일에 에러가 없으면
		{
			...
			$copied_file_name[$i] = $new_file_name.".".$file_ext; //2022_02_24_10_29_05_0.zip
			$uploaded_file[$i] = $upload_dir.$copied_file_name[$i]; //./data/2022_02_24_10_29_05_0.zip
	...
	{
		$sql = "insert into $table (id, cat, name, nick, subject, content, regist_day, hit, ";
		$sql .= " file_name_0, file_name_1, file_name_2, file_type_0, file_type_1, file_type_2, file_copied_0,  file_copied_1, file_copied_2) ";
		$sql .= " values('$cat','$userid', '$username', '$usernick', '$subject', '$content', '$regist_day', 0, ";
