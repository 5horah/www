...
<?
	...
	*/
	...
	$count = count($files["name"]); //업로드된 파일개수 3(파일개수)
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		if (!$upfile_error[$i]) //에러가 발생되지 않으면
		{
			...
			// 2019_03_22_10_07_15_0.jpg 실제 업로드 파일
			$uploaded_file[$i] = $upload_dir.$copied_file_name[$i];
			// ./data/2019_03_22_10_07_15_0.jpg

			if( $upfile_size[$i]  > 500000 ) { //500KB기준
				echo("
				<script>
			...
			if ( ($upfile_type[$i] != "image/gif") &&
				...
				($upfile_type[$i] != "image/pjpeg") && //익스 이미지 처리
				($upfile_type[$i] != "image/png") &&
				($upfile_type[$i] != "image/x-png")) //익스 이미지 처리
			{
				echo("
					<script>
	...
	if ($mode=="modify")
	{
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			$org_real_value = $copied_file_name[$i]; //2022_02_22_10_44_08_2.jpg
			if ($del_ok[$i] == "y")
			{
				$delete_field = "file_copied_".$i; //file_copied_0
				$delete_name = $row[$delete_field]; //$row[file_copied_0]; -> 2022_02_22_10_44_08_2.jpg
				
				$delete_path = "./data/".$delete_name; //'./data/2022_02_22_10_44_08_2.jpg'

				unlink($delete_path); //업로드파일삭제

				$sql = "update $table set $field_org_name = '$org_name_value', $field_real_name = '$org_real_value'  where num=$num";
				mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
			}
			else //첨부파일 삭제를 하지 않았을때
			{
				if (!$upfile_error[$i])
				{
					$sql = "update $table set $field_org_name = '$org_name_value', $field_real_name = '$org_real_value'  where num=$num";
